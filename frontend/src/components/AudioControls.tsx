...
export default function AudioControls({
  ...
  return (
    <div className="flex flex-wrap items-center justify-center md:justify-end gap-3 sm:gap-4 lg:gap-5">
      {/* Mic Button */}
      <button
        id="mic-toggle-btn"
      ...
      <div className="flex flex-col gap-1 items-center">
        ...
        <span className="text-[8px] text-text-muted uppercase tracking-widest font-extrabold">
      ...
      <button
        ...
        className="p-2 px-2.5 rounded-xl bg-white/[0.02] border border-border text-text-secondary hover:text-text-primary hover:bg-white/[0.05] hover:border-border-bright transition-all duration-300 cursor-pointer hover:scale-[1.03] flex items-center justify-center"
        title={theme === 'light' ? t('switchToDark', language) : t('switchToLight', language)}
      >
        {theme === 'light' ? (
          <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="text-accent-violet animate-bounce-subtle">
            <path d="M12 3a6 6 0 0 0 9 9 9 9 0 1 1-9-9Z"/>
          </svg>
        ) : (
          <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="text-accent-blue animate-spin-slow">
      ...
      <div className="flex items-center gap-2">
        <div className="lang-pill-container">
          ...
          <button
            ...
            className={`lang-pill ${language === 'es' ? 'lang-pill--active' : ''}`}
      ...
      <button
        ...
        className={`px-4 py-2 rounded-xl text-xs font-bold uppercase tracking-wider transition-all duration-300 relative overflow-hidden flex items-center gap-1.5 ${
          isDemo
            ? 'bg-gradient-to-r from-accent-violet to-indigo-600 text-white shadow-lg shadow-[rgba(139,92,246,0.3)] border border-transparent'
            : 'bg-white/[0.02] border border-border text-text-secondary hover:text-text-primary hover:bg-white/[0.05] hover:border-border-bright'
      ...
      <button
        id="reset-btn"
        onClick={onReset}
        className="p-2 px-3.5 rounded-xl text-xs font-bold uppercase tracking-wider text-text-muted bg-white/[0.02] border border-border hover:text-accent-rose hover:border-accent-rose hover:bg-[rgba(244,63,94,0.05)] transition-all duration-300 cursor-pointer hover:scale-[1.03]"
